...
extension NPTHomeViewController: UITableViewDelegate {
  func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    if indexPath.section == 0 {
      let vc = NEKaraokeListViewController()
...
extension NPTHomeViewController: UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    if let cell = tableView.dequeueReusableCell(withIdentifier: "NPTHomeCellIdentifier") as? NPTHomeCell {
      ...
      if indexPath.section == 0 {
        cell.backgroundImage.image = UIImage(named: "home_pink")
        cell.lottieView.animation = LottieAnimation.named("karaoke")
        cell.typeLabel.text = "Karaoke".localized
        cell.descriptionLabel.text = "Karaoke_Description".localized
