...
let NE = NEKaraokeNetwork.shared

/// 网络请求类
class NEKaraokeNetwork {
  let tag: String = "NEKaraokeNetwork"
  static let shared = NEKaraokeNetwork()
  // 配置信息
  ...
  func request(_ url: String,
    ...
    NEKaraokeLog.networkLog(
      tag,
      desc: "HTTPRequest\nUrl: \(url).\nMethod: \(method)\nHeaders: \(headers)\nBody: \(body?.prettyJSON ?? "")"
    )
    guard let URL = URL(string: url) else {
      NEKaraokeLog.errorLog(tag, desc: "\(url). Bad Url.")
      failed(makeError(NEKaraokeErrorCode.failed, "Bad url"))
      return
    }
    ...
    let dataTask = session.dataTask(with: request) { [weak self] data, response, error in
      ...
      if let error = error as NSError? {
        ...
        failed(makeError(NEKaraokeErrorCode.failed, "网络连接失败，请检查你的网络连接！"))
        return
      }
      ...
      guard httpResp.statusCode == 200 else {
        ...
        failed(makeError(NEKaraokeErrorCode.failed,
                         "Http error with status: \(httpResp.statusCode)"))
        return
      }
      ...
      guard let response = try? JSONSerialization.jsonObject(
        ...
        failed(makeError(NEKaraokeErrorCode.failed, "Cannot decode content data"))
        return
      }
      // 打印requestId
      if let requestId = response["requestId"] as? String {
        NEKaraokeLog.infoLog(self.tag, desc: "Url:\(url) RequestId:\(requestId)")
      }
      guard let code = response["code"] as? Int else {
        failed(makeError(NEKaraokeErrorCode.failed, "Empty code in response body!"))
        return
      }
      guard code == 200 else {
      ...
      NEKaraokeLog.infoLog(self.tag, desc: "Response data:\n\(response.prettyJSON)")
      success(dic)
    }
    dataTask.resume()
  }
  ...
  func request<T>(_ url: String,
    ...
    request(url, method: method, headers: headers, body: body) { dic in
      guard let dic = dic,
            dic.keys.count > 0 else {
        success(nil)
        return
      }
      success(NEKaraokeDecoder.decode(returnType, param: dic) ?? nil)
    } failed: { error in
      failed(error)
    }
  }
