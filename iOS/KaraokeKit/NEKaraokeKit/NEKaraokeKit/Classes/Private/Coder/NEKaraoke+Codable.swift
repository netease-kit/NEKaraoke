...
struct NEKaraokeCodingKeys: CodingKey {
...
extension KeyedDecodingContainer {
  func decode(_ type: [String: Any].Type, forKey key: K) throws -> [String: Any] {
    let container = try nestedContainer(keyedBy: NEKaraokeCodingKeys.self, forKey: key)
...
extension UnkeyedDecodingContainer {
  ...
  mutating func decode(_ type: [String: Any].Type) throws -> [String: Any] {
    let nestedContainer = try nestedContainer(keyedBy: NEKaraokeCodingKeys.self)
...
extension KeyedEncodingContainer {
  mutating func encodeIfPresent(_ value: [String: Any]?,
    ...
    var container = nestedContainer(keyedBy: NEKaraokeCodingKeys.self, forKey: key)
    for item in safeValue {
      if let val = item.value as? Int {
        try container.encodeIfPresent(
          val,
          forKey: NEKaraokeCodingKeys(stringValue: item.key)!
        )
...
extension UnkeyedEncodingContainer {
  ...
  mutating func encodeIfPresent(_ value: [String: Any]) throws {
    var container = nestedContainer(keyedBy: NEKaraokeCodingKeys.self)
    for item in value {
      if let val = item.value as? Int {
        try container.encodeIfPresent(
          val,
