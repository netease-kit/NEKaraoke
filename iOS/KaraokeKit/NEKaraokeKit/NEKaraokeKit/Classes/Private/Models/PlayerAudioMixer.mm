...
static inline int16_t FloatS16ToS16(float v) {
  ...
  if (v > 0) return v >= kMaxRound ? limits_int16::max() : static_cast<int16_t>(v + 0.5f);
...
void MixToFloatFrame(int16_t *src_data, int16_t *src2_data, int16_t *dst_data,
  ...
  if (samples_per_channel > kMaximumChannelSize || number_of_channels > kMaximumAmountOfChannels) {
    return;
  }
...
static void setAudioFrameVolume(int16_t *frame, float volume, int samples) {
  if (!frame || volume == 0.0) {
...
@implementation PlayerAudioMixer
...
+ (void)mixAudioFrameData:(int16_t *)mix_data
  ...
  MixToFloatFrame(mix_data, src_data2, mix_data, samples_per_channel, number_of_channels);
}

+ (void)setAudioFrameDataVolume:(int16_t *)to_data dataLen:(int)len volume:(float)volume {
  setAudioFrameVolume(to_data, volume, len);
}
