...
extension NEKaraokeKit: NECopyrightedEventHandler, NESongPreloadProtocol {
  public func onTokenExpired() {
    guard let eventHandler = copyrightedEventMediaHandler as? NEKaraokeCopyrightedMediaEventHandler
    else {
      return
    }
    NEKaraokeKit.shared().copyrightedMediaService?
      .getSongDynamicTokenUntilSuccess(success: { dynamicToken in
        ...
        NEKaraokeKit.shared().copyrightedMediaService?.calculateExpiredTime(timeExpired: expiresIn)
      })
  ...
  public func onPreloadStart(_ songId: String, channel: SongChannel) {
    for pointerListener in preloadProtocolListeners.allObjects {
      guard pointerListener is NEKaraokeCopyrightedMediaListener, let listener = pointerListener as? NEKaraokeCopyrightedMediaListener else { continue }
      if listener
        .responds(to: #selector(NEKaraokeCopyrightedMediaListener
            .karaoke_onPreloadStart(_:channel:))) {
        listener.karaoke_onPreloadStart?(songId, channel: channel)
      }
    }
  }
  ...
  public func onPreloadProgress(_ songId: String, channel: SongChannel, progress: Float) {
    for pointerListener in preloadProtocolListeners.allObjects {
      ...
      if listener
        .responds(to: #selector(NEKaraokeCopyrightedMediaListener
            .karaoke_onPreloadProgress(_:channel:progress:))) {
        listener.karaoke_onPreloadProgress?(songId, channel: channel, progress: progress)
      }
    }
  }
  ...
  public func onPreloadComplete(_ songId: String, channel: SongChannel, error: Error?) {
    for pointerListener in preloadProtocolListeners.allObjects {
      ...
      if listener
        .responds(to: #selector(NEKaraokeCopyrightedMediaListener
            .karaoke_onPreloadComplete(_:channel:error:))) {
        listener.karaoke_onPreloadComplete?(songId, channel: channel, error: error)
      }
    }
  }
  ...
  public func setCopyrightedMediaEventHandler(_ eventHandler: NEKaraokeCopyrightedMediaEventHandler?) {
    NEKaraokeLog.apiLog(kitTag, desc: "Add copyrighted event handle.")
    guard let eventHandler = eventHandler else {
      NEKaraokeLog.apiLog(kitTag, desc: "Add copyrighted event handle nil.")
      return
    }
