...
NSString *const kNEKaraokeReachabilityChangedNotification =
    @"kNEKaraokeReachabilityChangedNotification";

@interface NEKaraokeReachability ()
...
static NSString *karaokeReachabilityFlags(SCNetworkReachabilityFlags flags) {
  return [NSString
...
static void KaraokeReachabilityCallback(SCNetworkReachabilityRef target,
                                        SCNetworkReachabilityFlags flags, void *info) {
#pragma unused(target)

  NEKaraokeReachability *reachability = ((__bridge NEKaraokeReachability *)info);
...
@implementation NEKaraokeReachability

#pragma mark - Class Constructor Methods

+ (instancetype)reachabilityWithHostName:(NSString *)hostname {
  return [NEKaraokeReachability reachabilityWithHostname:hostname];
}
...
- (BOOL)startNotifier {
  ...
  if (SCNetworkReachabilitySetCallback(self.reachabilityRef, KaraokeReachabilityCallback,
                                       &context)) {
...
- (NSString *)currentReachabilityString {
  NEKaraokeNetworkStatus temp = [self currentReachabilityStatus];
...
- (NSString *)currentReachabilityFlags {
  return karaokeReachabilityFlags([self reachabilityFlags]);
}
...
- (void)reachabilityChanged:(SCNetworkReachabilityFlags)flags {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    [[NSNotificationCenter defaultCenter]
        postNotificationName:kNEKaraokeReachabilityChangedNotification
                      object:self];
  });
}
