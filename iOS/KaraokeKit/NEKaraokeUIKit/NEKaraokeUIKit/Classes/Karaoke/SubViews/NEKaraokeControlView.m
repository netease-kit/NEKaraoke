...
- (NEKaraokeControlButton *)voiceButton {
  if (!_voiceButton) {
    ...
    _voiceButton.title = NELocalizedString(@"调音");
    _voiceButton.icon = [UIImage karaoke_imageNamed:@"voice_icon"];
    __weak typeof(self) weakSelf = self;
    [_voiceButton addAction:^(NEKaraokeControlButton *button) {
      __strong typeof(weakSelf) self = weakSelf;
      if ([self.delegate respondsToSelector:@selector(onControlEvent:)]) {
...
- (NEKaraokeControlButton *)pauseButton {
  if (!_pauseButton) {
    ...
    _pauseButton.title = NELocalizedString(@"暂停");
    _pauseButton.selectedTitle = NELocalizedString(@"播放");
    _pauseButton.icon = [UIImage karaoke_imageNamed:@"pause_ico"];
    _pauseButton.selectedIcon = [UIImage karaoke_imageNamed:@"resume_ico"];
    __weak typeof(self) weakSelf = self;
    [_pauseButton addAction:^(NEKaraokeControlButton *button) {
      __strong typeof(weakSelf) self = weakSelf;
      if ([self.delegate respondsToSelector:@selector(onControlEvent:)]) {
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (NEKaraokeControlButton *)switchButton {
  if (!_switchButton) {
    ...
    _switchButton.title = NELocalizedString(@"切歌");
    _switchButton.icon = [UIImage karaoke_imageNamed:@"switch_ico"];
    __weak typeof(self) weakSelf = self;
    [_switchButton addAction:^(NEKaraokeControlButton *button) {
      __strong typeof(weakSelf) self = weakSelf;
      if ([self.delegate respondsToSelector:@selector(onControlEvent:)]) {
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (NEKaraokeControlButton *)orginalButton {
  if (!_orginalButton) {
    ...
    _orginalButton.title = NELocalizedString(@"原唱已关");
    _orginalButton.selectedTitle = NELocalizedString(@"原唱已开");
    ...
    __weak typeof(self) weakSelf = self;
    [_orginalButton addAction:^(NEKaraokeControlButton *button) {
