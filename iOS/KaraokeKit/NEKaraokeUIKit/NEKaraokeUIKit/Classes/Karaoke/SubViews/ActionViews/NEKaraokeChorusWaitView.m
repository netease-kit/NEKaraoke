...
- (void)setMainUserIcon:(NSString *)mainUserIcon {
  _mainUserIcon = mainUserIcon;
  [self.mainIconImageView sd_setImageWithURL:[NSURL URLWithString:mainUserIcon]];
}

- (void)setAttachUserIcon:(NSString *)attachUserIcon {
  _attachUserIcon = attachUserIcon;
  [self.attachIconImageView sd_setImageWithURL:[NSURL URLWithString:attachUserIcon]];
}
...
- (void)setMainUserName:(NSString *)mainUserName {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    self.userNameLabel.text =
        [NSString stringWithFormat:@"%@、%@ %@", mainUserName, self.attachUserName,
                                   NELocalizedString(@"请准备!")];
  });
}
...
- (void)setAttachUserName:(NSString *)attachUserName {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    self.userNameLabel.text =
        [NSString stringWithFormat:@"%@、%@ %@", self.mainUserName, attachUserName,
                                   NELocalizedString(@"请准备!")];
  });
}
...
- (UILabel *)waitLabel {
  if (!_waitLabel) {
    ...
    _waitLabel.text = NELocalizedString(@"伴奏加载中...");
