...
- (void)updateTime:(NSInteger)time {
  dispatch_async(dispatch_get_main_queue(), ^{
    NSString *content =
        [NSString stringWithFormat:@"%@ %zds", NELocalizedString(@"合唱匹配中"), time];
...
- (void)setUserIcon:(NSString *)userIcon {
  _userIcon = userIcon;
  [self.userImageView sd_setImageWithURL:[NSURL URLWithString:userIcon]];
}
...
- (UIButton *)soloButton {
  if (!_soloButton) {
    ...
    [_soloButton setTitle:NELocalizedString(@"独唱") forState:UIControlStateNormal];
...
- (UIButton *)joinButton {
  if (!_joinButton) {
    ...
    [_joinButton setTitle:NELocalizedString(@"加入合唱") forState:UIControlStateNormal];
