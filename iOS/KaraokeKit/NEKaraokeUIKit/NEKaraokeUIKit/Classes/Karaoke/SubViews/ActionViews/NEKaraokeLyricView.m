...
- (void)setContent:(NSString *)content lyricType:(NELyricType)type {
  ...
  [self
      preloadLyricCompleteBlock:^{
        ...
        [self.lyricView loadWithLyricModel:self.model];
      }
...
- (void)setPitchContent:(NSString *_Nonnull)pitchContent
  ...
  NSBundle *karaokeUIBundle = [NSBundle bundleForClass:[self class]];
  [self.compoentView
...
- (void)showOrHiddenSeekBtn:(NSInteger)leftTime {
  ...
  if (leftTime > 10 * 1000) {
    ...
    [self.seekBtn setTitle:[NSString stringWithFormat:@"%@(%zds)", NELocalizedString(@"跳过前奏"),
                                                      leftTime / 1000]
...
- (UIButton *)seekBtn {
  if (!_seekBtn) {
    ...
    [_seekBtn setTitle:[NSString stringWithFormat:@"%@(18s)", NELocalizedString(@"跳过前奏")]
              forState:UIControlStateNormal];
    ...
    [_seekBtn bk_whenTapped:^{
      __strong typeof(weakSelf) self = weakSelf;
      if (self.model.lineModels.count > 0) {
        NELyricLine *line = self.model.lineModels.firstObject;
        NSInteger startTime = line.startTime;
        if (line.words.count) {
          startTime = line.words.firstObject.startTime;
