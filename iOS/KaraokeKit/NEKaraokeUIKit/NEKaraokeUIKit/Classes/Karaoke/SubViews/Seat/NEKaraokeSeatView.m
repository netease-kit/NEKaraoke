...
@property(nonatomic, strong) NEKaraokeSongInfoModel *songModel;
@end
...
- (void)configSongModel:(NEKaraokeSongInfoModel *)songModel {
...
- (void)onReceiveChorusMessage:(enum NEKaraokeChorusActionType)actionType
  ...
  switch (actionType) {
    ...
    case NEKaraokeChorusActionTypeResumeSong: {
      self.songModel = songModel.chorusInfo;
    } break;
...
- (UICollectionView *)collectionView {
  if (!_collectionView) {
    CGFloat lineSpecing = (UIScreen.mainScreen.bounds.size.width - 30 - 50 * 7) / 6;
