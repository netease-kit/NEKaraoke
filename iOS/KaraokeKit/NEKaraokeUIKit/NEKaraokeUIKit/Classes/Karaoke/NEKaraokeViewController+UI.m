...
- (void)showAlert:(NSString *)title
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:NELocalizedString(@"取消")
...
- (void)showAcceptChorusLyricModule {
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    self.lyricActionView.chorusSongName = self.songModel.chorusInfo.songName;
    self.lyricActionView.chorusMainUserIcon = self.songModel.chorusInfo.icon;
    self.lyricActionView.chorusAttachUserIcon = self.songModel.chorusInfo.assistantIcon;
    self.lyricActionView.chorusMainUserName = self.songModel.chorusInfo.userName;
    self.lyricActionView.chorusAttachUserName = self.songModel.chorusInfo.assistantName;
...
- (void)showCancelInviteLyrricModule {
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    self.lyricActionView.waitSongName = self.songModel.chorusInfo.songName;
    self.lyricActionView.waitUserIcon = self.songModel.chorusInfo.icon;
    self.lyricActionView.waitUserName = self.songModel.chorusInfo.userName;
    self.lyricActionView.UserIconUrl = self.songModel.chorusInfo.icon;
  });
}

- (void)showNoLyricView:(NEKaraokeSongInfoModel *)songModel {
  dispatch_async(dispatch_get_main_queue(), ^{
    self.lyricActionView.noLyricUserIcon = songModel.icon;
...
- (void)fetchPickSongList {
  ...
  [[NEKaraokeKit shared]
      getOrderedSongsWithCallback:^(NSInteger code, NSString *_Nullable msg,
                                    NSArray<NEKaraokeOrderSongResult *> *_Nullable orderSongs) {
        dispatch_async(dispatch_get_main_queue(), ^{
