...
@implementation NEKaraokeChatViewMessage
...
- (void)drawAttributeLabel:(M80AttributedLabel *)label {
  ...
  if (self.isAnchor) {
    UIImage *authorIco = [UIImage karaoke_imageNamed:@"anthor_ico"];
  ...
  if (self.giftIcon.length) {
    UIImage *rewardIco = [UIImage karaoke_imageNamed:self.giftIcon];
...
- (NSAttributedString *)formatMessage {
  ...
  switch (_type) {
    ...
    case NEKaraokeChatViewMessageTypeReward: {
      [text setAttributes:@{
        ...
        NSFontAttributeName : [UIFont systemFontOfSize:14]
                    ...
                    range:_nickRange];
      [text setAttributes:@{
        NSForegroundColorAttributeName : [UIColor karaoke_colorWithHex:0xffffff],
        NSFontAttributeName : [UIFont systemFontOfSize:14]
      }
                    range:_textRange];
    } break;
    case NEKaraokeChatViewMessageTypeNotication: {
      [text setAttributes:@{
        NSForegroundColorAttributeName : [UIColor karaoke_colorWithHex:0xffffff],
        NSFontAttributeName : [UIFont systemFontOfSize:14]
      }
...
@interface NEKaraokeChatCell () {
  CGRect _preRect;
}

/// 消息模型
@property(nonatomic, strong) NEKaraokeChatViewMessage *model;
/// 富文本控件
@property(nonatomic, strong) M80AttributedLabel *attributedLabel;

@end

@implementation NEKaraokeChatCell
...
- (void)installWithModel:(NEKaraokeChatViewMessage *)model indexPath:(NSIndexPath *)indexPath {
  ...
  switch (model.type) {
    case NEKaraokeChatViewMessageTypeNormal:
    case NEKaraokeChatViewMessageTypeReward:
      self.contentView.backgroundColor = [UIColor karaoke_colorWithHex:0x000000 alpha:0.6];
      break;
    case NEKaraokeChatViewMessageTypeNotication: {
...
+ (NEKaraokeChatCell *)cellWithTableView:(UITableView *)tableView
  ...
  NEKaraokeChatCell *cell =
      [tableView dequeueReusableCellWithIdentifier:[NEKaraokeChatCell description]];
  if ([datas count] > indexPath.row) {
    NEKaraokeChatViewMessage *model = datas[indexPath.row];
...
@interface NEKaraokeChatView () <UITableViewDelegate, UITableViewDataSource> {
  CGRect _preRect;
}
...
@implementation NEKaraokeChatView
...
- (UITableView *)tableView {
  if (!_tableView) {
    ...
    [_tableView registerClass:[NEKaraokeChatCell class]
        forCellReuseIdentifier:[NEKaraokeChatCell description]];
...
- (void)processPendingMessages {
  ...
  for (NEKaraokeChatViewMessage *message in insert) {
...
- (void)addModels:(NSArray<NEKaraokeChatViewMessage *> *)models {
...
- (void)scrollToBottom:(NSArray<NEKaraokeChatViewMessage *> *)newModels {
  //    UIEdgeInsets insets = self.tableView.contentInset;
  //    if (insets.top != 0) {
  //        CGFloat height = 0;
  //        for (NEKaraokeChatViewMessage *model in newModels) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
  return [NEKaraokeChatCell heightWithIndexPath:indexPath datas:self.messages];
}

- (UITableViewCell *)tableView:(UITableView *)tableView
         cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  return [NEKaraokeChatCell cellWithTableView:tableView indexPath:indexPath datas:self.messages];
}
