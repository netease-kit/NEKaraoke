...
- (void)installWithModel:(NEKaraokeUIGiftModel *)model {
  self.icon.image = [UIImage karaoke_imageNamed:model.icon];
  self.info.attributedText = [self descriptionWithGift:model];
  self.info.textAlignment = NSTextAlignmentCenter;
}

- (NSAttributedString *)descriptionWithGift:(NEKaraokeUIGiftModel *)gift {
...
+ (NEKaraokeSendGiftCell *)cellWithCollectionView:(UICollectionView *)collectionView
                                        indexPath:(NSIndexPath *)indexPath
                                            datas:(NSArray<NEKaraokeUIGiftModel *> *)datas {
  ...
  if ([datas count] > indexPath.row) {
    NEKaraokeUIGiftModel *gift = datas[indexPath.row];
