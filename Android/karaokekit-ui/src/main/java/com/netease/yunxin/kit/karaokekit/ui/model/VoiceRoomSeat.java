...
public class VoiceRoomSeat implements Serializable, Parcelable {
  ...
  private OnSeatModel user;

  public VoiceRoomSeat(int index) {
    this(index, Status.INIT, Reason.INIT, null);
  }

  public VoiceRoomSeat(int index, int status, int reason, OnSeatModel user) {
    this.index = index;
    this.status = status;
    this.reason = reason;
    this.user = user;
  ...
  @Nullable
  public OnSeatModel getUser() {
    return user;
  }

  public void setUser(OnSeatModel user) {
    this.user = user;
  }
  ...
  protected VoiceRoomSeat(Parcel in) {
    ...
    this.user = (OnSeatModel) in.readSerializable();
  }
  ...
  public void kick() {
    if (status == Status.AUDIO_CLOSED_AND_MUTED || status == Status.AUDIO_MUTED) {
      status = Status.FORBID;
    } else {
      status = Status.INIT;
    }
  ...
  public void leave() {
    if (status == Status.AUDIO_MUTED || status == Status.AUDIO_CLOSED_AND_MUTED) {
      status = Status.FORBID;
    } else {
      status = Status.INIT;
    }
  ...
  public boolean isSameAccount(String account) {
    return !TextUtils.isEmpty(account) && user != null && account.equals(user.getAccount());
  }
